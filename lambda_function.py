...
def get_admin_info_by_email(email):
    ...
    conn.request("GET", f"/api/v1/organizations/{organization_id}/admins", headers=headers)
...
def update_meraki_admin_permission(admin_info, permission):
    ...
    if success:
        ...
        logger.error(f"Failed to update admin ID: {admin_id}, Response: {data}")
    return success
...
def lambda_handler(event, context):
    ...
    if not event_ts:
        ...
        return {
            'statusCode': 400,
            'body': json.dumps({'message': 'No event_ts found in the event payload'})
        }
    
    # イベントタイムスタンプの重複チェック
    if event_ts == last_processed_event_ts:
        logger.warning(f"Duplicate event detected: {event_ts}, skipping execution.")
        return {
            'statusCode': 200,
    ...
    if email_match and permission_match:
        ...
        if not post_message_to_slack(message):
            logger.error("Failed to notify Slack after updating admin permission.")
